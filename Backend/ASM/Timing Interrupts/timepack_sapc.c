...
int starttimer()
{
  ...
  count = (int)inpt(TIMER0_COUNT_PORT);
  count |= (int)(inpt(TIMER0_COUNT_PORT) << 8);
  count_start = COUNTS_PER_TICK - count;
  timer_running = TRUE;
  return T_OK;
}
...
int stoptimer( int *interval )
{
  ...
  count = (int)inpt(TIMER0_COUNT_PORT);
  count |= (int)(inpt(TIMER0_COUNT_PORT) << 8);
  count_end = COUNTS_PER_TICK - count;
  count = count_end - count_start;
  *interval = (tickcount - startcount) * USECS_PER_TICK + (count * 55000.0 / COUNTS_PER_TICK);
  timer_running = FALSE;
  return T_OK;
}
